...
class Vertex: #the Vertex class contains 3 attributes: .x, .y, .incident_half_edges
    ...
    def sort_incident(self) -> None: #sorts the incident edges in counter-clockwise order
...
class HalfEdge: # the half-edge class contains the following attributes: .origin, .destination, .twin, .next, .prev, .face; note that half edges are, therefore, directed
    def __init__(self, origin = None, destination = None):
        ...
        self.destination: Optional[Vertex] = destination    # Vertex where this half-edge ends
    ...
    @cached_property
    def angle(self) -> float: #this computes the angle between this half-edge and the x-axis
        dx = self.destination.x - self.origin.x
        dy = self.destination.y - self.origin.y
        angle = math.atan2(dy, dx)
        return angle if angle >= 0 else angle + 2 * math.pi
    
    def _is_point_to_left(self, point) -> bool: #decide if the the given point (a tuple) is to the left of this half edge
...
class Face: #the Face class has three attributes: .start_half_edge, .inner_components_half_edges, .is_external
    def __init__(self):
        self.start_half_edge: Optional[HalfEdge] = None      # A half-edge bounding the outer face cycle -- note that this remains None for the external face
        self.inner_components_half_edges = [] # Half edges from each of the inner cycles of the face
        self.is_external: Optional[bool] = None # True if the face is external; False otherwise
    ...
    def _vertices_in_cycle(self, start_half_edge): #yields the vertices around the cycle starting at the given edge
    ...
    def _edges_in_cycle(self, start_half_edge): #yields the edges around the cycle starting at the given edge
        if not start_half_edge:
            return
    ...
    def outer_vertices(self) -> Iterator[Vertex]: #yields the vertices that bound this face 
        if not self.start_half_edge:
            return
        
        yield from self._vertices_in_cycle(start_half_edge = self.start_half_edge)

    def inner_vertices(self) -> Iterator[List[Vertex]]: #vertices of the holes inside the face
    ...
    def outer_edges(self) -> Iterator[HalfEdge]: #yields the edges that bound this face 
        if not self.start_half_edge:
            return
        
        yield from self._edges_in_cycle(start_half_edge = self.start_half_edge)

    def inner_edges(self) -> Iterator[List[HalfEdge]]: #edges of the holes inside the face
        if not self.inner_components_half_edges:
            return
...
class DCEL: #The DCEL class has .vertices, .half_edges, and .faces as its attributes
    ...
    def _add_edge_pair(self, v1, v2) -> Tuple[HalfEdge]: #adds the half edge v1v2 and the half edge v2v1 to the DCEL; note that v1 and v2 are in the Vertex class 
        he1 = HalfEdge()
        he2 = HalfEdge()
        he1.origin = v1
        he1.destination = v2
    ...
    def _set_next_prev_pointers(self) -> None: #this sets the prev and next pointers of half edges
    ...
    def _build_dcel_vertices_and_half_edges_from_graph(self, G) -> None: #given a networkx graph, we compute the vertices and half edges of our DCEL first
    ...
    def _find_all_cycles(self) -> List[List[HalfEdge]]: #we now compute cycles in the DCEL; each cycle is represented by a list of half edges
        unprocessed = set(self.half_edges)
        cycles = []
        
        while unprocessed:
            start_edge = unprocessed.pop() # Pick any unprocessed half-edge to start a cycle
            cycle = []
            current = start_edge
    ...
    def _left_bottom_edge(self, cycle) -> HalfEdge: #Returns the edge which has the left-bottom vertex of the cycle as its origin
        left_bottom_edge = cycle[0]
        left_bottom_vertex = left_bottom_edge.origin

        for edge in cycle:
            v = edge.origin
            if v.x < left_bottom_vertex.x: #if it is strictly to the left, update our values
                left_bottom_vertex = v
                left_bottom_edge = edge
            elif v.x == left_bottom_vertex.x: #if its equal, check if the y-coordinate is lower
    ...
    def _is_inner_cycle(self, cycle) -> bool: #this check if the cycle is an inner circle or an outer circle; this is simply by checking what angle the edges of the cycle which has the left-bottom-vertex as its origin and destination make
        he1 = self._left_bottom_edge(cycle)
        he2 = he1.prev

        he1_vector = ((he1.origin).x - (he1.destination).x, (he1.origin).y - (he1.destination).y)
        he2_vector = ((he2.origin).x - (he2.destination).x, (he2.origin).y - (he2.destination).y)
    ...
    def _determine_cycle_with_edge(self, cycles, edge, point) -> int: #this gives the index of the cycle which contains a given edge and has the point to its left
        ...
        he.origin = v1
        he.destination = v2
        if not he._is_point_to_left(point): #if this direction does not have the point to its left, then flip it around
            he.origin = v2
            he.destination = v1
    ...
    def _compute_connectivity_graph(self, cycles, vertical_edges, vertical_edges_tree): #this constructs a graph on the cycles to compute which inner cycle contains which collection of outer cycles
        C = nx.Graph()
        ...
        for i, cycle in enumerate(cycles):
            if not self._is_inner_cycle(cycle): #if this is an outer cycle, compute the left bottom vertex of it
                left_bottom_vertex = (self._left_bottom_edge(cycle)).origin
                left_bottom_vertex = (left_bottom_vertex.x, left_bottom_vertex.y)
                if left_bottom_vertex[0] > 0: #NOTE: we assume here that the external face is the only one which has a non-positive x-coordinate
                    _, bisected_edge_index = ray_shooting(point = left_bottom_vertex, tree = vertical_edges_tree, is_left = 1, is_down = 0, is_ray_horizontal = 1) #shoot a ray to the left
                    bisected_edge = vertical_edges[bisected_edge_index] #this is the edge it intersects
                    j = self._determine_cycle_with_edge(cycles, bisected_edge, left_bottom_vertex) #compute which cycle it belongs to
                    C.add_edge(i,j) # add this edge
        return C

    def _construct_faces_from_cycles(self, cycles, vertical_edges): #we finally construct the faces of our DCEL
        vertical_edges_tree = create_interval_tree(vertical_edges, is_vertical_tree = True)
        
        H = self._compute_connectivity_graph(cycles, vertical_edges, vertical_edges_tree) # Build containment graph H using our ray shooting method
        ...
        for component in components: # Create faces from connected components
            component_cycles = [cycles[i] for i in component] # Get cycles in this component
            ...
            if len(inner_cycles_in_component) == 1: # The inner cycle is the outer boundary of this face; if there is none, then it is the external face
            ...
            elif len(inner_cycles_in_component) == 0 and len(outer_cycles_in_component) > 0:
    ...
    def _generate_3d_polygons(self): #generate all the polyhedra that make up the histogram 
        polygons_3d = []
        ...
        for face in self.faces: #iterate over faces
            ...
            face_vertices_2d = list(face.outer_vertices()) # Get 2D vertices of the face boundary using face's vertices method
            
            if len(face_vertices_2d) < 3:
                continue  # Skip degenerate faces

            bottom_vertices_3d = [(v.x, v.y, 0) for v in face_vertices_2d] # Bottom face (z = 0)
            polygons_3d.append({'vertices': bottom_vertices_3d,'type': 'bottom','face': face,'height': 0})
            
            top_vertices_3d = [(v.x, v.y, height) for v in face_vertices_2d] # Top face (z = height)  
            polygons_3d.append({'vertices': top_vertices_3d,'type': 'top', 'face': face,'height': height})
            ...
            for i in range(n):
                ...
                polygons_3d.append({'vertices': side_face,'type': 'side','face': face,'height': height})
            ...
            for hole_vertices_2d in face.inner_vertices():
                ...
                polygons_3d.append({'vertices': bottom_hole_3d,'type': 'bottom_hole','face': face,'height': 0})
                
                # Top hole (subtract from top face)
                top_hole_3d = [(v.x, v.y, height) for v in hole_vertices_2d]
                polygons_3d.append({'vertices': top_hole_3d, 'type': 'top_hole','face': face,'height': height})
                ...
                for i in range(n_hole):
                    ...
                    polygons_3d.append({'vertices': inner_side_face,'type': 'inner_side','face': face,'height': height})
    ...
    def _set_axis_properties_3d(self, ax): #Set 3D axis properties for better visualization
    ...
    def plot_histogram_polyhedron(self, alpha=0.15, show_wireframe=True, face_colors=None):
        ...
        polygons_3d = self._generate_3d_polygons() # Generate all 3D polygons for the polyhedron
        ...
        for poly_info in polygons_3d: # Second pass: assign colors to all polygons based on their DCEL face
        ...
        if face_color_map:
            ...
            if heights:
                sm = plt.cm.ScalarMappable(cmap=plt.cm.viridis, norm=plt.Normalize(vmin=min(heights), vmax=max(heights)))
                sm.set_array([])
